...
public class QnaBoardEditProc implements CmpInter {
	...
	public String exec(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(title != null) {
			buff.append(" , qtitle = '" + title + "' ");
		}
		if(body != null) {
			buff.append(" , qbody = '" + body + "' ");
		}
		...
		int cnt = qDao.editQnaBoard(qno, psql);
		
		// 결과값에 따라 처리하고
		if(cnt != 1) {
			// 실패한 경우
			view = "/camp24/qnaBoard/qnaBoardEdit.cmp?qno=" + sno + "&nowPage=" + spage;
		}
